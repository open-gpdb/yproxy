...
yproxy --config test/regress/conf/yproxy_vacuum.yaml --test-mode &> /var/log/yproxy_vacuum.log &

# Wait for yproxy to become available
i=0
while (! [ -S /tmp/yproxy.sock ] || ! [ -S /tmp/yproxy_old.sock ] || ! [ -S /tmp/yproxy_identical.sock ] || ! [ -S /tmp/yproxy_prefix.sock ] || ! [ -S /tmp/yproxy_vacuum.sock ] ) && [ $i -lt 20 ]; do sleep 1; i=$(($i+1)) ; done
( [ -S /tmp/yproxy.sock ] && [ -S /tmp/yproxy_old.sock ] && [ -S /tmp/yproxy_identical.sock ] && [ -S /tmp/yproxy_prefix.sock ] && [ -S /tmp/yproxy_vacuum.sock ] ) || ( echo 'yproxy startup failed'; exit 2 )
