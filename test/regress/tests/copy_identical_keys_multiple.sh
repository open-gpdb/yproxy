#!/bin/bash
set -e

echo 'Sample encrypted data, identical key' | yp-client --config test/regress/yproxy_second_bucket.yaml -l fatal put --encrypt 'encrypted_file_identical_key_1'
echo 'Sample encrypted data, identical key' | yp-client --config test/regress/yproxy_second_bucket.yaml -l fatal put --encrypt 'encrypted_file_identical_key_1'

psql -h pg -U postgres -d test -c "INSERT INTO yezzey.yezzey_virtual_index VALUES ('/encrypted_file_identical_key_1'), ('/encrypted_file_identical_key_2')"

yp-client copy --config test/regress/yproxy.yaml --old-config test/regress/yproxy_second_bucket.yaml --encrypt --decrypt --port 5432 --confirm --log-level error ''
yp-client --config test/regress/yproxy.yaml -l fatal list ''
yp-client --config test/regress/yproxy.yaml -l fatal cat --decrypt 'encrypted_file_identical_key_1'
yp-client --config test/regress/yproxy.yaml -l fatal cat --decrypt 'encrypted_file_identical_key_2'
