...
yp-client copy --config test/regress/conf/yproxy.yaml --old-config test/regress/conf/yproxy_second_bucket.yaml --encrypt --decrypt --port 5432 --confirm --log-level error 'encrypted_file_identical_key'
yp-client --config test/regress/conf/yproxy.yaml -l fatal list ''
yp-client --config test/regress/conf/yproxy.yaml -l fatal cat --decrypt 'encrypted_file_identical_key'
