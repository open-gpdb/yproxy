...
func TestDeleteMsg(t *testing.T) {
	...
	msg := message.BuildDisposalMessage("myname/mynextname", 5432, 42, true, true)
	body := msg.Encode()

	assert.Equal(body[8], byte(message.MessageTypeDelete))

	msg2 := message.DisposalMessage{}
	msg2.Decode(body[8:])
...
func TestDelete2Msg(t *testing.T) {
	...
	msg := message.BuildDisposalPrefixMessage("trash/prefix", true, false)
	body := msg.Encode()

	assert.Equal(body[8], byte(message.MessageTypeDelete2))

	msg2 := message.DisposalPrefixMessage{}
	msg2.Decode(body[8:])
